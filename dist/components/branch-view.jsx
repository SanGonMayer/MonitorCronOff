...
export default function BranchView() {
    ...
    const handleConnect = async (hostname, ip) => {
    ...
    const filteredHosts = hosts?.filter((host) => host.hostname.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <CardContent className="pt-6">
          <div className="grid gap-6">
            <div className="grid gap-2">
              ...
              <Input id="branch-number" type="number" min="1" max="999" placeholder="Ej: 42" value={branchNumber} onChange={(e) => setBranchNumber(e.target.value)} className="rounded-xl"/>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="hosts">Hosts de la Filial</Label>
              <Textarea id="hosts" placeholder="Ingrese nombres de host (uno por línea o separados por comas)" value={hostInput} onChange={(e) => setHostInput(e.target.value)} className="min-h-[150px] rounded-xl"/>
            </div>

            <Button className="bg-gradient-to-r from-[#8A2BE2] to-[#9932CC] hover:from-[#7B1FA2] hover:to-[#8E24AA] text-white rounded-xl flex gap-2 items-center w-full sm:w-auto justify-center" onClick={handleAddHostsByBranch} disabled={isSubmitting}>
        ...
        <CardContent className="pt-6">
          <div className="grid gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar hosts..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 rounded-xl"/>
            </div>

            {isLoading ? (<div className="flex justify-center py-8">
                <Loader2 className="h-8 w-8 animate-spin text-[#8A2BE2]"/>
              </div>) : filteredHosts && filteredHosts.length > 0 ? (<div className="rounded-xl border overflow-hidden">
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {filteredHosts.map((host) => (<TableRow key={host.id}>
                        ...
                        <TableCell>
                          <Button variant="outline" size="sm" className="flex items-center gap-1 rounded-xl" onClick={() => handleConnect(host.hostname, host.ip_address)} disabled={connecting === host.hostname}>
                            {connecting === host.hostname ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<Terminal className="h-4 w-4"/>)}
                            Conectar
                          </Button>
                        </TableCell>
                      </TableRow>))}
              ...
              </div>) : (<div className="text-center py-8 text-muted-foreground bg-muted/10 rounded-xl">
